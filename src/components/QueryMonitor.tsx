...
const QueryMonitor: React.FC<QueryMonitorProps> = ({
  ...
  const handleKillProcess = async (sessionId: number) => {
    if (
      !confirm(`Tem certeza que deseja encerrar o processo SPID ${sessionId}?`)
    ) {
      return;
    }
          ...
          <thead>
            <tr>
              ...
              <th onClick={() => handleSort("reads")}>
                Leituras{" "}
                {sortField === "reads" && (sortDirection === "asc" ? "↑" : "↓")}
              </th>
          ...
          <tbody>
            {filteredQueries.map((query) => (
              ...
              >
                ...
                <td>{query.status}</td>
                <td>
                  {query.blockingSessionId > 0 ? query.blockingSessionId : "-"}
                </td>
